...
توجه کنید که ما `jax.numpy as jnp` را import می‌کنیم که یک رابط مشابه NumPy فراهم می‌کند.
...
* NumPy از یک باینری از پیش‌ساخته برای اعمال کسینوس روی آرایه‌ای از اعداد اعشاری استفاده می‌کند
* این باینری روی CPU دستگاه محلی اجرا می‌شود

##### با JAX

حالا بیایید با JAX امتحان کنیم.

```{code-cell}
x = jnp.linspace(0, 10, n)
```

بیایید همین رویه را زمان‌بندی کنیم.
...
به مفسر Python اجازه می‌دهد جلوتر از محاسبات عددی اجرا شود.

برای کدهایی که زمان‌بندی نمی‌شوند، می‌توانید خط حاوی `block_until_ready` را حذف کنید.
```

و دوباره زمان‌بندی کنیم.
...
همچنین، معمولاً اجرای دوم به دلیل کامپایل JIT سریع‌تر از اولی است.

این به این دلیل است که حتی توابع داخلی مانند `jnp.cos` نیز JIT-کامپایل می‌شوند --- و
اجرای اول شامل زمان کامپایل است.

چرا JAX می‌خواهد توابع داخلی مانند `jnp.cos` را به جای ارائه نسخه‌های از پیش‌کامپایل‌شده مانند NumPy، JIT-کامپایل کند؟

دلیل این است که کامپایلر JIT می‌خواهد روی *اندازه* آرایه مورد استفاده (و همچنین نوع داده) تخصص پیدا کند.

اندازه برای تولید کد بهینه‌شده اهمیت دارد زیرا موازی‌سازی کارآمد نیاز به تطابق اندازه وظیفه با سخت‌افزار موجود دارد.

می‌توانیم ادعای تخصص JAX روی اندازه آرایه را با تغییر اندازه ورودی و مشاهده زمان‌های اجرا تأیید کنیم.
...
تفاوت دیگری بین NumPy و JAX این است که JAX به طور پیش‌فرض از اعداد اعشاری 32 بیتی استفاده می‌کند.
...
طراحان JAX تصمیم گرفتند آرایه‌ها را تغییرناپذیر کنند زیرا JAX از سبک برنامه‌نویسی تابعی استفاده می‌کند که در ادامه درباره آن بحث می‌کنیم.
...
به عبارت دیگر، JAX یک سبک
[برنامه‌نویسی تابعی](https://en.wikipedia.org/wiki/Functional_programming)
را فرض می‌کند.
...
این گراف‌های محاسباتی توصیف می‌کنند که چگونه یک مجموعه داده ورودی به خروجی تبدیل می‌شود.

آن‌ها ذاتاً خالص هستند.
...
تولید اعداد تصادفی در JAX به طور قابل توجهی با الگوهای موجود در NumPy یا MATLAB تفاوت دارد.

در ابتدا ممکن است نحو را نسبتاً پرمخاطب بیابید.

اما نحو و معناشناسی برای حفظ سبک برنامه‌نویسی تابعی که به تازگی مورد بحث قرار دادیم، ضروری است.
...
در رابط برنامه‌نویسی قدیمی تولید اعداد تصادفی NumPy (که از MATLAB تقلید می‌کند)، تولید با حفظ وضعیت سراسری پنهان کار می‌کند.
...
قدرت کامپایلر JIT JAX را در ترکیب با سخت‌افزار موازی {ref}`در بالا <jax_speed>` دیدیم، هنگامی که `cos` را روی یک آرایه بزرگ اعمال کردیم.
...
با این حال، با JAX، ترفند دیگری در آستین داریم --- می‌توانیم *کل* تابع را JIT-کامپایل کنیم، نه فقط عملیات‌های منفرد.
...
فرض کنید تابعی داریم که آمارهای خلاصه را برای یک آرایه منفرد محاسبه می‌کند:
...
تابع `summary` برای یک آرایه منفرد نوشته شده بود، و `vmap` به‌طور خودکار
